...
(function () {
  ...
  const SYSTEM_PROMPT = `أنت A3M Bot، المساعد الذكي لـ A3M Print للطباعة في الجزائر. شخصيتك: روبوت بيكسل آرت لطيف ومفيد 🤖
منتجات: تيشرتات، هوديات، أكواب سيراميك (أبيض/أسود)، أكواب ستاربكس ورقية (أبيض/أسود)، قبعات كاب وباكيت، حقائب، بطاقات، ملصقات، ورق A3/A4/A5، نقش ليزر.
طباعة: حرارية، تطريز، DTG، UV، ليزر، فينيل، سيلك سكرين، أوفست.
قواعد: رد بلغة المستخدم. كن مختصراً وودوداً. إذا طلب تصميماً أضف:
[GENERATE_DESIGN:{"type":"tshirt","title":"TITLE","subtitle":"sub","color":"#2563eb","bg":"#0b0f19","textColor":"#fff"}]`;
  ...
  const PAGE_SUGS = {
    ...
    products: [{label:"👕 التيشرتات",text:"أسعار التيشرتات؟"},{label:"🧥 الهوديات",text:"أنواع الهوديات؟"},{label:"☕ الأكواب",text:"أنواع الأكواب المتوفرة؟"},{label:"🖨️ أفضل طباعة",text:"أفضل نوع طباعة للملابس؟"},{label:"📦 الكميات",text:"خصم للكميات الكبيرة؟"}],
  ...
  function detectCtxKey(text) {
    ...
    if(/(منتج|تيشرت|هودي|كوب|قبعه|mug|cup|starbucks)/.test(n)) return "products";
  ...
  function resolveRobotSrc() {
    ...
    _robotSrc = "assets/images/robot.webp"; // المسار الافتراضي
    return _robotSrc;
  }
...
@media(max-width:480px){#a3m-bot-chat{width:calc(100vw - 28px);right:-8px;max-height:540px}#a3m-bot-wrap{right:14px;bottom:14px}.a3m-messages{max-height:300px}}
  ...
  function greetUser(){
    ...
    setTimeout(()=>{appendMessage("bot","👋 أهلاً! أنا **A3M Bot** 🤖 مساعدك الذكي في A3M Print — كيف أساعدك اليوم؟\n\n🛍️ المنتجات · 🎨 التصميم · 💰 الأسعار · 🖨️ الطباعة");showSuggestions(null);},320);
  }
