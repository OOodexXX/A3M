...
(function () {
  ...
  function saveToCart() {
    // ✅ FIX: استخدم السعر المحسوب الفعلي من updateDesignPrice بدل 1500 الثابت
    if (typeof window.updateDesignPrice === "function") window.updateDesignPrice();
    const price = window._a3mCurrentDesignPrice || 500;
    const prodType = C().canvasProdType || "tshirt";
    const prodNames = { tshirt:"تيشرت", mug:"كوب", hat:"قبعة", bag:"حقيبة",
                        paper:"ورقة", hoodie:"هودي", phone:"جراب", note:"دفتر" };
    const prodLabel = prodNames[prodType] || prodType;
    const name = "تصميم مخصص — " + prodLabel;
    const item = { id:"custom-design-"+Date.now(), name, emoji:"🎨", price, qty:1 };
    if (typeof window.cart !== "undefined") window.cart.push(item);
    else { try { const k="a3m_cart"; const c=JSON.parse(localStorage.getItem(k)||"[]"); c.push(item); localStorage.setItem(k, JSON.stringify(c)); } catch(e){} }
    if (typeof updateCart === "function") updateCart();
    if (typeof showToast === "function") showToast("🛒 تمت الإضافة — " + price.toLocaleString() + " دج", "success");
  }
