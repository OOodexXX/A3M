...
(function () {
  ...
  // ✅ Guard: THREE must be loaded before this IIFE executes
  if (typeof THREE === "undefined") {
    var _p3dPoll = setInterval(function() {
      if (typeof THREE !== "undefined") {
        clearInterval(_p3dPoll);
        window._p3dInitialized = true;
        console.log("A3M 3D: THREE now available");
      }
    }, 50);
    console.warn("A3M 3D: THREE not loaded, preview3d disabled");
    return;
  }
  window._p3dInitialized = true;
  ...
  function open3DViewer() {
    ...
    // ✅ FIX: Wait for modal to render before init (so clientWidth is correct)
    requestAnimationFrame(() => {
      setTimeout(() => {
        ...
        startSync();
        updateProductTabs();
        // Apply current design immediately
        setTimeout(applyDesignTexture, 200);
      }, 50);
    });
  }
  ...
  function loadProductModel(type) {
    ...
    switch (type) {
      ...
      // ── OBJ model cases ──
      case "mug_obj":      loadObjModel("./assets/models/mug.obj",          PRODUCT_COLORS.mug_obj);      break;
      case "starbucks_mug":loadObjModel("./assets/models/starbucks_mug.obj", PRODUCT_COLORS.starbucks_mug); break;
      case "cap_obj":      loadObjModel("./assets/models/Cap.obj",           PRODUCT_COLORS.cap_obj);      break;
      case "tshirt_obj":   loadObjModel("./assets/models/tshirt.obj",        PRODUCT_COLORS.tshirt);       break;
      default:             buildTshirt();
    }
    currentProduct = type;
    applyDesignTexture();
  ...
  function applyDesignTexture() {
    ...
    // ✅ FIX: Create a scaled-down texture canvas for THREE.js
    // (1200x1200 canvas works fine as CanvasTexture, just ensure it's ready)
    if (!srcCanvas.width || !srcCanvas.height) return;

    try {
      if (designTex) designTex.dispose();
      // Use the canvas directly — THREE handles any size
      designTex = new THREE.CanvasTexture(srcCanvas);
      designTex.needsUpdate = true;
      // r128 uses encoding (not colorSpace)
      if (THREE.sRGBEncoding) designTex.encoding = THREE.sRGBEncoding;
      designTex.flipY = false;  // ✅ prevent upside-down texture
    ...
    } catch(e) { console.warn("3D texture error:", e); }
  }
  ...
  // ════════════════════════════════════════════════════════════
  // OBJ MODEL LOADER — for mug.obj, Cap.obj, starbucks_mug.obj
  // ════════════════════════════════════════════════════════════

  function loadObjModel(path, color) {
    setStatus("جاري تحميل الموديل...");
    ...
    // Use THREE.OBJLoader if available, otherwise fallback to a placeholder
    const useLoader = typeof THREE.OBJLoader !== "undefined";
    if (!useLoader) {
      console.warn("A3M 3D: OBJLoader not found — using placeholder geometry");
      _buildObjPlaceholder(color);
      setStatus("");
      return;
    }
    ...
    loader.load(
      ...
      function onLoad(obj) {
        ...
        // Apply material + mark print zone
        obj.traverse(child => {
          if (child.isMesh) {
            child.material = mat;
        ...
        // Add print zone plane on front face
        const planeGeo = new THREE.PlaneGeometry(size.x * scale * 0.6, size.y * scale * 0.55);
        const planeMat = new THREE.MeshStandardMaterial({
          color: 0xffffff, transparent: true, opacity: 0.0,
          map: designTex || null,
        });
        printZone = new THREE.Mesh(planeGeo, planeMat);
        printZone.position.set(0, 0.15, size.z * scale * 0.52);
        obj.add(printZone);
      ...
      function onProgress(xhr) {
        if (xhr.lengthComputable) {
          ...
          setStatus(pct < 100 ? "تحميل " + pct + "%" : "");
        }
      },
      function onError(err) {
        console.error("A3M 3D: OBJ load failed", err);
        _buildObjPlaceholder(color);
        setStatus("⚠ تعذّر تحميل الموديل");
      }
    );
  }

  function _buildObjPlaceholder(color) {
    // Simple box as fallback
    const geo = new THREE.BoxGeometry(2, 2.5, 1.2);
    const mat = new THREE.MeshStandardMaterial({ color: color || 0xcccccc, roughness: 0.7 });
    model = new THREE.Mesh(geo, mat);
    model.castShadow = true;
    // Print zone
    const pGeo = new THREE.PlaneGeometry(1.4, 1.6);
    const pMat = new THREE.MeshStandardMaterial({ color: 0xffffff, transparent: true, opacity: 0, map: designTex || null });
    printZone = new THREE.Mesh(pGeo, pMat);
    printZone.position.set(0, 0, 0.61);
    model.add(printZone);
    scene.add(model);
    applyDesignTexture();
  }
  ...
  function _buildBoxPlaceholder(color) {
    var geo = new THREE.BoxGeometry(2, 2.5, 1.2);
    var mat = new THREE.MeshStandardMaterial({ color: color || 0xcccccc, roughness: 0.7 });
    var box = new THREE.Mesh(geo, mat);
    box.castShadow = true;
    var pGeo = new THREE.PlaneGeometry(1.4, 1.6);
    var pMat = new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 });
    var printZoneMesh = new THREE.Mesh(pGeo, pMat);
    printZoneMesh.position.set(0, 0, 0.62);
    box.add(printZoneMesh);
    printZone = printZoneMesh;
    model = box;
    scene.add(model);
    applyDesignTexture();
  }
